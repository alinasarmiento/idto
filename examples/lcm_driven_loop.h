...
class LcmDrivenLoop {
 public:
  ...
  LcmDrivenLoop(drake::lcm::DrakeLcm* drake_lcm,
    ...
    if (!backup_drive_channel.empty()) {
      state_sub_ = std::make_unique<drake::lcm::Subscriber<dairlib::lcmt_robot_output>>(
  ...
  void Simulate(double end_time = std::numeric_limits<double>::infinity()) {
	...
	// (greysar) Handle any messages that come in the next 1 ms (will clear old messages, since DrakeLcmInterface buffer is 1).
	drake_lcm_->HandleSubscriptions(1);
 ...
 private:
  ...
  std::unique_ptr<drake::lcm::Subscriber<dairlib::lcmt_robot_output>> state_sub_;
